...
#warning 这里是第一次调用 HMStatusFrame的 status的set方法
        frame.status = status;
...
- (void)showNewStatusesCount:(int)count
{
    ...
    label.height = 35;
    label.x = 0;
    label.y = 64 - label.height;   //应该是先藏在导航栏的下面   拖拽的时候就能显示出来了
...
#warning    这里调用HMStatusCell中  statusFrame 的set方法
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 1.差距        //yyyy tableview一次好多数据,所以一个屏幕显示不全,得用contentSize来计算
    CGFloat delta = scrollView.contentSize.height - scrollView.contentOffset.y;
        // 刚好能完整看到footer的高度  //这个值就是屏幕高度减去底部tabbar的高度
    CGFloat sawFooterH = self.view.height - self.tabBarController.tabBar.height;
    // 2.如果能看见整个footer
    if (delta <= (sawFooterH - 0)) {//yyy 这就表示到了当前显示的"最后一页"
        // 进入上拉刷新状态
